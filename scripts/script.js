...
function listHotels(jsonObject) {
    ...
    for (var hotel = 0; hotel < jsonObject["resultsCount"]; hotel++)
    {
        ...
        for (var itinerary = 0; itinerary < itinerariesCount; itinerary++)
        {
            ...
            var htmlToAppend = "<div class='itinerary container row'>";
            htmlToAppend += "<img class ='hero-image col-sm-4' src=" + currentItineraries['imageUrl'][0] + ">";
            htmlToAppend += "<div class='hotel-details row col-sm-7'>";
            htmlToAppend += "<div class='col-sm-'><h2>" + currentItineraries['name'] + "</h2>" + getStars(currentItineraries['starRating']) + "</div>";
            htmlToAppend += "<div><div class='amenities'>" + getAmenities(currentItineraries['amenities']) + "</div></div>";
            htmlToAppend += "<div><div class='address'>Address:" + currentItineraries['address'] + "</div>";
            htmlToAppend += "<label>"+  +"</label><a class='slide-cover' target='_blank' href='singleavail.html?cid=" + sessionId + "&iid=" + currentItineraries['itineraryId'] + "'> View Deal</a ></div>";
            htmlToAppend += "</div>";
            htmlToAppend += "</div>";
